...
for p = 2:npa
    dper1 = ((x(p)-x(p-1))^2+(t(p)-t(p-1))^2)^0.5;
    perimeter = perimeter + dper1;
end

% A_fuel = sum(t(1:npa/2))/(np);

A_fuel = 0;
for n_fuel = 2:(npa+1)/2
    if x(n_fuel) <= 0.5
        A_fuel = A_fuel+abs(((x(n_fuel+1)-x(n_fuel-1))/2)*(t(n_fuel)-t(npa-n_fuel)));
    end
end

A = 0;
for n_t = 2:(npa+1)/2
    A = A+abs(((x(n_t+1)-x(n_t-1))/2)*(t(n_t)-t(npa-n_t)));
end
